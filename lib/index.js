...
PostgresStorage = (function() {
    ...
    return self.db.query('BEGIN TRANSACTION', function(err) {
      ...
      return async.waterfall([
        function(done) {
          ...
          return self.db.query(sql, [id], function(err) {
            return done(err);
          });
        }, function(done) {
          var sql;
          sql = "delete from " + self.settings.table + " where id = $1";
          return self.db.query(sql, [id], done);
        }
      ], function(err, res) {
        if (err) {
          return self.db.query('ROLLBACK TRANSACTION', function() {
  ...
  PostgresStorage.prototype.getFileById = function(id, callback) {
    return this.db.query("select * from " + this.settings.table + " where id = $1", [id], function(err, res) {
      if (err) {
        return callback(err);
      }
  ...
  PostgresStorage.prototype._stream = function(file, res, callback) {
    var bufferSize, man;
    bufferSize = 16384;
    man = new LargeObjectManager(this.db);
  ...
  PostgresStorage.prototype.downloadById = function(id, res, callback) {
    ...
    return self.db.query('BEGIN TRANSACTION', function(err) {
      ...
      return self.getFileById(id, function(err, file) {
        if (err) {
          return callback(err);
        }
        return self._stream(file, res, callback);
      });
    });
  };
